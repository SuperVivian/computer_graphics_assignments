...
void SceneParser::parsePerspectiveCamera () {
	...
	getToken (token);
	assert (!strcmp (token, "up"));
	Vec3f up = readVec3f (); getToken (token);
...
void SceneParser::parseLights () {
	char token[MAX_PARSER_TOKEN_LENGTH]; getToken (token); assert (!strcmp (token, "{"));
...
Group* SceneParser::parseGroup () {
	...
	while (num_objects > count) {
		getToken (token); if (!strcmp (token, "MaterialIndex")) {
			...
			assert (object != NULL);
			answer->addObject (count, object); count++;
		}
	}
	getToken (token); assert (!strcmp (token, "}"));
	// return the group 
...
Triangle* SceneParser::parseTriangle () {
	...
	Vec3f v0 = readVec3f (); getToken (token);
	assert (!strcmp (token, "vertex1"));
	Vec3f v1 = readVec3f (); getToken (token);
	assert (!strcmp (token, "vertex2"));
	Vec3f v2 = readVec3f (); getToken (token);
	assert (!strcmp (token, "}"));
	assert (current_material != NULL);
...
Group* SceneParser::parseTriangleMesh () {
	...
	while (1) {
		...
		if (c == EOF) { break; }
			...
			int f0, f1, f2; fscanf (file, "%d %d %d", &f0, &f1, &f2); fcount++;
		}
	...
	assert (file != NULL);
	int new_vcount = 0;
	int new_fcount = 0; while (1) {
		int c = fgetc (file); if (c == EOF) { break; }
		else if (c == 'v') {
			assert (new_fcount == 0);
			float v0, v1, v2;
			fscanf (file, "%f %f %f", &v0, &v1, &v2);
			verts[new_vcount] = Vec3f (v0, v1, v2); new_vcount++;
		}
		else if (c == 'f') {
			...
			assert (current_material != NULL);
			Triangle *t = new Triangle (verts[f0 - 1], verts[f1 - 1], verts[f2 - 1], current_material); answer->addObject (new_fcount, t); new_fcount++;
		}
		// otherwise, must be whitespace
	}
	delete[] verts;
...
Transform* SceneParser::parseTransform () {
	...
	getToken (token); while (1) {
		if (!strcmp (token, "Scale")) {
			matrix *= Matrix::MakeScale (readVec3f ());
		}
		else if (!strcmp (token, "UniformScale")) {
			float s = readFloat (); matrix *= Matrix::MakeScale (Vec3f (s, s, s));
		}
			...
			for (int j = 0; j < 4; j++) {
				for (int i = 0; i < 4; i++) {
					float v = readFloat (); matrix2.Set (i, j, v);
				}
			}
			getToken (token);
			assert (!strcmp (token, "}"));
			...
			object = parseObject (token); break;
		} getToken (token);
	}
	assert (object != NULL);
	getToken (token);
...
Vec3f SceneParser::readVec3f () {
	float x, y, z; int count = fscanf (file, "%f %f %f", &x, &y, &z);
	if (count != 3) {
...
Vec2f SceneParser::readVec2f () {
	float u, v; int count = fscanf (file, "%f %f", &u, &v);
	if (count != 2) {
...
int SceneParser::readInt () {
	int answer; int count = fscanf (file, "%d", &answer);
	if (count != 1) {
