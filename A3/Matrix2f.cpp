...
Matrix2f::Matrix2f( const Vec2f& v0, const Vec2f& v1, bool setColumns )
{
	if( setColumns )
	{
		setCol( 0, v0 );
...
Vec2f Matrix2f::getRow( int i ) const
{
	return Vec2f
	(
		m_elements[ i ],
...
void Matrix2f::setRow( int i, const Vec2f& v )
{
	m_elements[ i ] = v.x();
	m_elements[ i + 2 ] = v.y();
}

Vec2f Matrix2f::getCol( int j ) const
{
	int colStart = 2 * j;
...
void Matrix2f::setCol( int j, const Vec2f& v )
{
	int colStart = 2 * j;
