...
RayTracer::RayTracer (SceneParser *s, int max_bounces, float cutoff_weight, bool shadows, Vec3f g, bool isG)
{
	...
	if (!isG) {
		...
		Group* group = this->scene->getGroup ();
		grid = new Grid (group->getBoundingBox (), g.x (), g.y (), g.z ());//Create grid （用grid来代替场景中的物体）
		group->insertIntoGrid (grid, NULL);//insert primitive into grid 分别调用各个物体的对应插入函数
		grid->setMaterial (scene->getMaterial (0));
		group->setIsGrid (isG);//是否和grid进行射线检测
...
{
	if (bounces < 0) return Vec3f (0, 0, 0);//weight怎么用。。。。呢？
